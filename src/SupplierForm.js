...
const SupplierForm = () => {
    ...
    const submitNewSupplier = (e) => {
        ...
        fetch(`${domain}/supplier/new`, {
            method: "POST",
            // headers: { Token: JSON.parse(localStorage.getItem("login")).token },
            body: JSON.stringify({ ...supplier, products })
        })
    ...
    return (
        <div className="p-5">
            {submitted && <>
                <h1>Thank you for Registerating</h1>
                <a className="navbar-brand" href="/">
                    <button className="btn btn-info"> <i class="material-icons">Go To Login </i></button>
                </a>
            </>}
